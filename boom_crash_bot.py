...
def check_entry(df5: pd.DataFrame, bias: str,
    ...
    bb_squeeze = last["bb_width"] <= np.percentile(
        recent_bw, BB_SQUEEZE_PERCENTILE)
...
def format_diagnostic() -> str:
    ...
    if any_close:
        lines.append(
            "⚡ <b>Some markets are close to signalling — stay alert!</b>")
    else:
        lines.append(
            "<i>No markets are near a signal right now. Filters protecting you.</i>")
...
# ─────────────────────────────────────────────────────────────
#  CONNECTION TEST
#  Run once on startup to verify Deriv WebSocket is reachable.
# ─────────────────────────────────────────────────────────────
...
async def run_bot():
    print("=" * 60)
    print("  BOOM & CRASH SIGNAL BOT — ADAPTIVE EDITION v2")
    ...
    HEARTBEAT_EVERY = 3600   # send a heartbeat every 1 hour
    ...
    send_telegram(
        ...
        "📈 Boom 300 | Boom 500 | Boom 1000\n"
        "📉 Crash 300 | Crash 500 | Crash 1000\n\n"
        "🔌 Testing connection to Deriv...\n"
        "⏳ Please wait for connection result below."
    )

    # ── Connection test before entering loop ─────────────────
    connected = await test_connection()
    if not connected:
        send_telegram(
            "⚠️ <b>Starting anyway — will retry each scan cycle.</b>\n"
            "Check connection errors above."
        )
    ...
    send_telegram(
        ...
        "✅ <b>Relaxed settings:</b>\n"
        "  • Scanning 24/7 — no session filter\n"
        "  • BB squeeze 35th percentile\n"
        "  • 30M bias: EMA + RSI only\n"
        "  • Min confluence: 2/3\n\n"
        "📲 <b>Commands:</b>\n"
        ...
        f"⏱ First scan in {CHECK_INTERVAL // 60} minutes.\n"
        "━━━━━━━━━━━━━━━━━━━━━━━\n"
    ...
    while True:
        ...
        # Diagnostic — only after first scan has completed
        if first_scan_done and now_ts - last_diagnostic >= DIAGNOSTIC_EVERY:
            send_telegram(format_diagnostic())
            last_diagnostic = now_ts

        # Hourly heartbeat — confirms bot is alive
        if now_ts - last_heartbeat >= HEARTBEAT_EVERY:
            total_today = sum(s["daily_signals"]
                              for s in market_state.values())
            send_telegram(
                f"💓 <b>Bot Heartbeat — {scan_time}</b>\n"
                f"Status: Running ✅\n"
                f"Signals today: {total_today}\n"
                f"Next scan: ~{CHECK_INTERVAL // 60}m\n"
                f"<i>Type /diag for filter details.</i>"
            )
        ...
        scan_errors = []   # collect errors across all markets this cycle
        scanned_count = 0    # how many markets attempted a real scan
        ...
        for name, info in MARKETS.items():
            ...
            try:
                ...
                if live_price is None:
                    err_msg = f"{name}: WebSocket tick failed — check API connection"
                    print(f"  {err_msg}")
                    state["last_block_reason"] = "No data — WebSocket tick returned None"
                    state["scans_blocked"] += 1
                ...
                if df5.empty or df30.empty or len(df5) < 60 or len(df30) < 30:
                    err_msg = (f"{name}: insufficient candles "
                               f"(5M={len(df5)} 30M={len(df30)})")
                    print(f"  {err_msg}")
                    state["last_block_reason"] = (
                        f"No data — got {len(df5)} 5M candles, "
                        f"{len(df30)} 30M candles (need 60 and 30)"
                    )
                    state["scans_blocked"] += 1
                    scan_errors.append(err_msg)
                    continue

                # Patch last candle with live tick
                ...
                if not sig:
                    ...
                    state["last_block_reason"] = (
                        f"No Compression — BB/ATR squeeze not detected "
                ...
                if sig["score"] < min_score:
                    print(
                        f"  {name}: score {sig['score']} < {min_score} required")
                    state["last_block_reason"] = (
                ...
                err_msg = f"{name}: EXCEPTION — {e}"
                print(f"  {err_msg}")
                state["last_block_reason"] = f"Exception — {str(e)[:80]}"
                scan_errors.append(err_msg)
